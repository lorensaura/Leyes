...
- **Correr `scripts/contenido_practica_2026-07.sql` en Supabase.** 117
  Alternativas + 32 Memorice ya redactadas y auditadas desde 2026-07-24
  (cubren Contractual, Extracontractual y Precontractual), nunca
  cargadas. Correrlo antes de generar contenido nuevo de esos dos modelos
  para cualquiera de esas tres materias, o se corre el riesgo de
  duplicar preguntas ya escritas sobre el mismo punto legal.
- **Subir las 46 flashcards nuevas de Extracontractual** (ver arriba) a
  Airtable y correr el sync.
...
- **Commitear y pushear lo que quedó de la sesión pasada**: hay cambios
  sin commitear ahora mismo (`CLAUDE.md` reorganizado, el fix del script
  de sync, docs nuevos, PDFs regenerados). Laura los pushea con GitHub
  Desktop.
...
- **Qué hacer con las 4 tablas nuevas de Airtable** (`Aplicación`,
  `Detección de error`, `Justificación`, `Discriminación MC`) y los 16
  ítems huérfanos de Evaluación que quedaron ahí: ¿migrar los 105 ítems
  reales del banco hardcodeado hacia esas tablas (para poder editar
  Evaluación sin tocar código), o borrarlas y descartar la idea?
- **¿Deberían verse las preguntas `materia = 'transversal'` de
  Alternativas también al filtrar una sola área?** Hoy solo aparecen
  bajo "Todas". Es un cambio de una línea por ítem si la respuesta es sí.
