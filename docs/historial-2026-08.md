# Historial detallado de sesiones, agosto 2026

> **Archivo, no doc vivo.** Esto es la bitácora sesión a sesión que hasta
> el 2026-08-12 vivía dentro de `docs/camino-a-beta.md`, movida acá tal
> cual (copia exacta, sin resumir ni borrar nada) para que ese doc pudiera
> quedar corto y legible. **Para saber qué está pendiente hoy, usa
> `docs/camino-a-beta.md`, no este archivo.** Este archivo sirve para
> encontrar el detalle fino de cómo se llegó a un estado (qué ítem exacto
> se generó, qué se verificó contra qué línea del manual, qué se descartó
> y por qué) cuando haga falta ese nivel de detalle. No se actualiza in
> place: es un corte congelado al 2026-08-12, más lo que se le vaya
> agregando después como nuevas entradas fechadas (acá sí tiene sentido
> acumular por fecha, a diferencia de `camino-a-beta.md`).

---

# Camino al beta de Digesto (versión hasta 2026-08-12, ver nota arriba)
...
> de git si hace falta recuperarlos. Última actualización: 2026-08-12.
...
- Materias más allá de Contractual/Extracontractual/Precontractual
  ...
  Responsabilidad. **Template de manuales para materias nuevas ya escrito**
  (`docs/script_apuntes.md`, 2026-08-12, piloto Bienes), pero el andamiaje
  de código para que una materia nueva funcione en Práctica queda
  pendiente. Hallazgo del 2026-08-12 (agente Explore sobre
  `app/alternativas.html`): el filtro de "Materia" (Responsabilidad/Bienes/
  etc.) hoy **no filtra nada de verdad**, solo el filtro de "Área" dentro
  de Responsabilidad filtra contenido; si se prendiera Bienes en el menú
  tal cual está hoy, se seguiría mostrando todo el contenido de
  Responsabilidad mezclado. **Idea de Laura para resolverlo de raíz:**
  particionar el contenido por materia de forma real en Supabase (tablas
  separadas por materia, o una columna/etiqueta de materia que sí se use
  como filtro real en la consulta, no solo como texto libre como hoy) para
  que, si una alumna elige por ejemplo "Acto Jurídico" y "Bienes", la
  consulta a Supabase solo traiga esas dos particiones y no el resto. Sin
  diseñar todavía (esquema de tablas, si es 1 tabla con columna `materia`
  indexada o varias tablas), queda para cuando se retome esta materia en
  serio.
