...
del body), encabezado/pie con DIGESTO + título + número de página (sin
encabezado en la portada). Requiere Google Chrome instalado y los paquetes
`websocket-client` y `PyMuPDF`.
"""
import base64
import json
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory() as profile_dir:
        ...
        try:
            ...
            # portada (decidido 2026-07-28, ver docs/pdf.md). Chrome sanitiza
            # cualquier <script> dentro de headerTemplate, así que no hay
            # forma de ocultarlo condicionalmente por página dentro de un
            # solo printToPDF: se generan dos versiones completas (una sin
            # encabezado, otra con) y se combinan tomando la página 1 de la
            # primera y el resto de la segunda. Los márgenes son idénticos en
            # ambas, así que la paginación no cambia entre una y otra.
            header_template = f'''
