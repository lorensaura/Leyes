...
- **Estado (2026-07-11, fin del día):** llave `ANTHROPIC_API_KEY` configurada en Vercel (Production) y probada — Laura corrió una interrogación real completa (15 preguntas + caso de 3). Del feedback de esa prueba se corrigieron dos cosas en `api/_interrogador-prompt.js` y `app/interrogador.html`: (1) preguntas compuestas que se respondían solas (ej. "¿cuáles son los daños en REX? y compare daño emergente con lucro cesante" — la segunda parte revelaba la respuesta a la primera; ahora la regla 1 lo prohíbe explícitamente con un ejemplo), y (2) el modo práctica mostraba `**` literales en vez de formato; ahora hay una convención de marcado (`~~tachado~~` = incorrecto, `__subrayado__` = correcto, `**negrita**` = lo que faltó) que la página traduce a HTML real. **Este fix quedó commiteado en local (no pusheado)** — falta: (a) que Laura suba con GitHub Desktop, (b) volver a probar una interrogación real para confirmar que ambos arreglos funcionaron como se espera.
