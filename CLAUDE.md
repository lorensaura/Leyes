...
## Decisiones tomadas
- **Los 3 SQL pendientes de Práctica/Memorice ya están corridos en Supabase (confirmado 2026-07-28):** `scripts/supabase_schema_practica.sql`, `scripts/supabase_schema_practica_metodo_b.sql` y `scripts/memorice_literales_2026-07-28.sql` (artículos de transcripción literal que faltaban en Contractual y Código de Comercio — para los 2 de Código de Comercio, el texto se verificó contra un espejo de leychile.cl porque el sitio oficial no cargó en esa sesión; vale la pena que Laura los confirme puntualmente contra leychile.cl).
- **Un artículo de Memorice puede pertenecer a más de una materia, en una sola fila (2026-07-28).** Al preparar de correr `scripts/contenido_practica_2026-07.sql` se encontró que 9 artículos nuevos reusaban el `id` de uno ya cargado en otra materia (ej. art. 44, relevante tanto en Contractual como Extracontractual) — como `id` es la clave única de toda la tabla, esas filas se habrían perdido en silencio (`on conflict (id) do nothing`). Se descartó duplicar con un id distinto (mostraría el artículo dos veces al filtrar Área = "Todas") y en cambio `materia` ahora puede guardar varios valores separados por coma (`'contractual,extracontractual'`) en una sola fila; `app/alternativas.html` usa la función nueva `perteneceAArea` para filtrar por pertenencia en vez de igualdad exacta. Detalle completo, y el pendiente sin decidir sobre las preguntas `transversal` de Alternativas, en `docs/practica.md`.
- **Contenido de Práctica y de Evaluación completado para Extracontractual y Precontractual (2026-07-24/25).** Módulo Práctica (`app/alternativas.html`): terminados los últimos 5 ejes de Extracontractual (25/25) y los 10 ejes de Precontractual completos (10/10, primera vez que esta materia tiene contenido), más un lote de 10 preguntas transversales. **`scripts/contenido_practica_2026-07.sql` (Alternativas + Memorice de esta tanda) ya está corrido y confirmado en Supabase (2026-07-28).** Por separado, en Airtable (pipeline del Interrogador IA) se encontró y limpió un problema real: la base "Digesto Precontractual" tenía `Temas`/`Flashcards` sobrantes con `materia = "Responsabilidad contractual"` (copia inservible de cuando se duplicó la base, nunca leída por el sync), y solo 19 `Preguntas_Evaluacion` sin ninguna `Opciones_MC`. Se borró ese sobrante, se creó Precontractual bien (10 `Temas` + 59 `Flashcards` en la base principal `Digesto`), se generaron 100 preguntas nuevas releyendo el manual eje por eje (10 por eje, con `Elementos_Clave` y `Opciones_MC` reales para las 20 de tipo `discriminacion_mc`), y se corrió `sync_airtable_supabase.py` — esto sí quedó confirmado en Supabase (119 preguntas y 59 flashcards de Precontractual, `publicado = true`). **Todo el contenido jurídico nuevo de esta tanda (Práctica y Evaluación) está sin revisar por Laura.** Detalle completo, IDs de bases y convenciones de nomenclatura en `.claude/handoff/handoff-20260725.md` (gitignored, snapshot puntual).
...
- **Alternativas y Memorice se quedan solo en Supabase, sin pasar por Airtable (2026-07-27).** Se evaluó llevarlas al patrón Airtable→sync (como Flashcards/Preguntas_Evaluacion), con el diseño ya resuelto (una sola base para las tres materias), pero Laura prefirió mantenerlas como están: son ítems más simples que no necesitan quedar perfectos a la primera ("modo pasta"), así que ella redacta el SQL directo y lo corre en Supabase — agregar Airtable de por medio sería más fricción, no menos. Ver `docs/contenido-airtable-supabase.md`.
